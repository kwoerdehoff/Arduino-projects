...
#ifdef ETHERSHIELD_DEBUG
    sprintf(_DEBUG, "EncClient::connect() DEBUG C. _sock = %d",
            _sock);
#endif
...
#ifdef ETHERSHIELD_DEBUG
    sprintf(_DEBUG, "EncClient::connect() DEBUG D. _sock = %d", _sock);
#endif
...
void EncClient::write(uint8_t b) {
  if (_sock != 255)
    send(_sock, &b, 1);
}

void EncClient::write(const char *str) {
  if (_sock != 255)
    send(_sock, (const uint8_t *)str, strlen(str));
}

void EncClient::write(const uint8_t *buf, size_t size) {
  if (_sock != 255)
    send(_sock, buf, size);
}
...
// by EncServer::available() to null, or use it as the condition in an
